...
var Game = {
    ...
    // Função que inverte jogadores

	nextPlayer(){
		this.currentPlayer = this.currentPlayer === 'X' ? 'O' : 'X';
	},
    // Função que seta rounds e renderiza jogada
    ...
    // Função que valida se jogo acabou e vencedor
	isGameOver(){
    ...
    // Função de Renderização e visualização de infos no front (Vencedor, Jogador Atual)
	render(){
		var winner = this.isGameOver();
		divElement.textContent = winner ? `Vencedor: ${winner}` : `Jogador atual: ${this.currentPlayer}`;
